...
const config: Config = {
  ...
  theme: {
    extend: {
      ...
      fontFamily: {
        sans: ["var(--font-poppins)"], // Changed to Poppins to match layout
        inter: "var(--font-poppins)", // Mapping inter to poppins as well for compatibility
      },
      fontSize: {
        0: ["0px", "0px"],
        xl: ["1.125rem", "2rem"],
  ...
  plugins: [
    ...
    plugin(function ({ addBase, addComponents, addUtilities }: any) {
      ...
      addComponents({
        ...
        ".btn-secondary": {
          "@apply btn bg-secondary-1 border-secondary-1 text-n-1 fill-n-1 hover:bg-secondary-1/90 hover:border-transparent":
            {},
        },
        ".btn-red": {
